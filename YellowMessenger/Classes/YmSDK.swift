...
public class ChatController: UIViewController, WKScriptMessageHandler, WKNavigationDelegate, SFSpeechRecognizerDelegate, AVSpeechSynthesizerDelegate {
    ...
    let currentBundle = Bundle.init(for: ChatController.self)

    var closeIcon: UIImage? {
        guard let bundleUrl = currentBundle.url(forResource: "YellowMessenger", withExtension: "bundle") else {
            return nil
        }
        return UIImage.init(named: "yt_close_icon", in: Bundle.init(url: bundleUrl), compatibleWith: nil)
    }
    var micIcon: UIImage? {
        guard let bundleUrl = currentBundle.url(forResource: "YellowMessenger", withExtension: "bundle") else {
            return nil
        }
        return UIImage.init(named: "yt_mic_icon", in: Bundle.init(url: bundleUrl), compatibleWith: nil)
    }
    ...
    override public func viewDidLoad() {
        ...
        let enableHistory = YmBotPlugin.shared.configData["EnableHistory"] != nil && YmBotPlugin.shared.configData["EnableHistory"] == "true"
        let urlString = """
        https://yellowmessenger.github.io/pages/dominos/mobile.html?botId=\(YmBotPlugin.shared.configData["BotId"] ?? "")&enableHistory=\(enableHistory ? "true" : "false")&hideTitleBar=false&ym.payload=\(payload)
        """
    ...
    func restartSpeechTimer() {
        ...
        self.detectionTimer = Timer.scheduledTimer(withTimeInterval: 1.5, repeats: false, block: { (timer) in
            ...
            self.button.setImage(self.micIcon, for: .normal)
